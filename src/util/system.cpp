...
const char * const BITCOIN_CONF_FILENAME = "roundcoin.conf";
...
#else
    const char* pszModule = "roundcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Roundcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Roundcoin";
#else
    // Unix
    return pathRet / ".roundcoin";
#endif
#endif
}
